...
docker container run -d --name rutorrent-filebot -p 8080:8080 -v volume_data:/data -e DEBUG=${DEBUG:-false} -e GEOIP_ACCOUNT_ID=${GEOIP_ACCOUNT_ID} -e GEOIP_LICENSE_KEY=${GEOIP_LICENSE_KEY} ${DOCKER_USERNAME:-local}/${IMAGE_NAME}:${TRAVIS_COMMIT}
sleep 60
